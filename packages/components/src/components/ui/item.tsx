...
const itemVariants = cva(
  "group/item flex w-full flex-wrap items-center rounded-lg border text-sm transition-colors duration-100 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] [a]:hover:bg-muted [a]:transition-colors",
  {
    variants: {
      variant: {
...
function ItemDescription({ className, ...props }: ComponentProps<"p">) {
  return (
    <p
      ...
      className={cn(
        "text-muted-foreground line-clamp-2 text-left text-sm leading-normal font-normal [&>a:hover]:text-primary group-data-[size=xs]/item:text-xs [&>a]:underline [&>a]:underline-offset-4",
        className,
      )}
      {...props}
    />
